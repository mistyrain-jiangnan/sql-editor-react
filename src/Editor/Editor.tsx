...
function Editor(
  ...
  ref: React.ForwardedRef<RefEditorInstance>
) {
  const {
    width = "100%", // 默认宽度
    height = "100%", // 默认高度
  ...
  const $editor = useRef<monaco.editor.IStandaloneCodeEditor>(); // 编辑器引用
  ...
  const refElement = useCallback(
    (node: HTMLDivElement) => {
      if (node !== null && monacoEditor) {
        const finalOptions = { ...mergedOptions, ...handleEditorWillMount() };
        container.current = node;
        const model = monaco.editor.createModel(formatSQL(val), language);
        $editor.current = editor.create(
          node,
          {
            model,
...
// eslint-disable-next-line react-refresh/only-export-components
export default React.forwardRef<RefEditorInstance, MonacoEditorProps>(Editor);
